...
class TestBatch(unittest.TestCase):
    ...
    def test_assemble_material_graph(self):
        ...
        self.assertTrue(np.allclose(g_assembled.bond_atom_indices[:n_bond1],
                                    self.g1.bond_atom_indices))
        self.assertTrue(np.allclose(g_assembled.bond_atom_indices[n_bond1:],
                                    self.g2.bond_atom_indices + n_atom1))

        g_assembled2 = assemble_material_graph([self.g1.as_list(),
                                                self.g2.as_list()])
        ...
        self.assertTrue(np.allclose(g_assembled_mg.bond_atom_indices[:n_bond1],
                                    self.g1.bond_atom_indices))
        self.assertTrue(np.allclose(g_assembled_mg.bond_atom_indices[n_bond1:],
                                    self.g2.bond_atom_indices + n_atom1))
    ...
    def test_sequential_assemble_list(self):
        ...
        self.assertTrue(
            MaterialGraph.from_list(g3) == MaterialGraph.from_list(g4)
        )
    ...
    def test_materialgraph_efs(self):
        graphs = [self.g1, self.g2]
        energies = [0.1, 0.2]
        forces = [np.random.normal(size=(2, 3)),
                  np.random.normal(size=(2, 3))]
        stresses = [np.random.normal(size=(3, 3)),
        ...
        self.assertListEqual(list(outshapes[0]), [2, ])
