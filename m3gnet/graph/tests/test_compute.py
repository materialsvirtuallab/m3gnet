...
def _loop_indices(bond_atom_indices,
                  pair_dist,
                  cutoff=4.0):
    bin_count = np.bincount(bond_atom_indices[:, 0],
                            minlength=bond_atom_indices[-1, 0] + 1)
    indices = []
    start = 0
        ...
        for i in range(n_site):
            for j in range(n_site):
                ...
                if (di <= threebody_cutoff) and (dj <= threebody_cutoff):
                    cos.append(
                        vi.dot(vj) / np.linalg.norm(vi) / np.linalg.norm(vj))
        start_index += n_site
    return cos
...
class TestCompute(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.s1 = Structure(Lattice.cubic(3.17), ["Mo", "Mo"],
                       [[0, 0, 0], [0.5, 0.5, 0.5]])
        cls.s2 = Structure(Lattice.cubic(3), ["Mo", "Fe"],
                       [[0, 0, 0], [0.5, 0.5, 0.5]])
    ...
    def test_compute_pair_vec(self):
        ...
        np.testing.assert_array_almost_equal(np.sort(d),
                                             np.sort(d2))

    def test_compute_dist_angle(self):
        g2 = tf_compute_distance_angle(self.g1.as_list(),
                                       )
        _, _, _, d2 = self.s1.get_neighbor_list(r=5.0)

        np.testing.assert_array_almost_equal(
            np.sort(g2[Index.BONDS].numpy().ravel()),
            np.sort(d2))

        cos_loop = _calculate_cos_loop(self.g1.as_list())

        cos = g2[Index.THETA]
        np.testing.assert_array_almost_equal(cos_loop,
                                             cos)
