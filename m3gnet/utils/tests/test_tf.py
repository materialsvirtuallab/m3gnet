...
class TestTF(unittest.TestCase):
    def test_calculations(self):
        np.testing.assert_array_almost_equal(
            get_segment_indices_from_n([2, 3]),
            [0, 0, 1, 1, 1]
        )

        np.testing.assert_array_almost_equal(
            get_range_indices_from_n([2, 3]),
            [0, 1, 0, 1, 2]
        )
    ...
    def test_segments(self):
        ...
        res = unsorted_segment_fraction(x,
                                        [0, 0, 1, 1],
                                        2)
    ...
    def test_broadcast(self):
        ...
        self.assertTupleEqual(tuple(broadcast_states_to_bonds(graph).shape),
                              (11, 2))

        graph[Index.N_ATOMS] = np.array([3, 5])
        self.assertTupleEqual(tuple(broadcast_states_to_atoms(graph).shape),
                              (8, 2))
    ...
    def test_register(self):
        ...
        register(TestClass)
        self.assertTrue(
            'm3gnet>TestClass' in tf.keras.utils.get_custom_objects())
        ...
        self.assertTrue(
            'TestClass2' in tf.keras.utils.get_custom_objects())
