...
class TestMath(unittest.TestCase):
    ...
    def test_spherical_bessel_harmonics_function(self):
        ...
        sbf = SphericalBesselFunction(max_l=3, cutoff=5.0, max_n=3,
                                      smooth=False)
        res = sbf(r)
        res2 = sbf.rbf_j0(r, cutoff=5.0, max_n=3)
        self.assertTrue(
            np.allclose(res[:, :3].numpy().ravel(), res2.numpy().ravel()))

        self.assertTrue(res.numpy().shape == (10, 9))

        sbf2 = SphericalBesselFunction(max_l=3, cutoff=5.0, max_n=3,
                                       smooth=True)
        ...
        res_shf = shf(
            costheta=np.linspace(-1, 1, 10),
            phi=np.linspace(0, 2 * np.pi, 10)
        )

        self.assertTrue(res_shf.numpy().shape == (10, 9))
        combined = combine_sbf_shf(res, res_shf, max_n=3, max_l=3,
                                   use_phi=True)

        self.assertTrue(combined.shape == (10, 27))

        res_shf2 = SphericalHarmonicsFunction(max_l=3, use_phi=False)(
            costheta=np.linspace(-1, 1, 10), phi=np.linspace(0, 2 * np.pi,
                                                               10)
        )
        combined = combine_sbf_shf(res, res_shf2, max_n=3, max_l=3,
                                   use_phi=False)
