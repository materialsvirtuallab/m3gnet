...
class TestModel(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.structure = Structure(Lattice.cubic(3.30),
                      ['Mo', 'Mo'],
                      [[0, 0, 0], [0.5, 0.5, 0.5]])
    ...
    def test_m3gnet(self):
        ...
        vals = self.model.predict_structures(
            [self.mol, self.mol]).numpy().ravel()
    ...
    def test_potential(self):
        ...
        self.assertTrue(np.allclose(f.numpy().ravel(),
                                    np.zeros(shape=(2, 3)).ravel(),
                                    atol=1e-3))
        self.assertTrue(np.allclose(np.diag(s[0].numpy()).ravel(),
                                    np.array([[28.519585, 28.519585,
                                               28.519585]]),
                                    atol=1e-2))
